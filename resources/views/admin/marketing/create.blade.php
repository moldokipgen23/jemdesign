...
@section('title', 'Add Marketing Section')

@section('content')
<div class="admin-page-header">
    <h1 class="admin-page-header__title">Add Marketing Section</h1>
</div>

<form action="{{ route('admin.marketing.store') }}" method="POST" id="sectionForm">
    @csrf
    ...
    <div class="admin-card" style="max-width:800px">
        ...
        <div class="admin-card__body">
            <div class="admin-form-group">
                ...
                <input type="text" name="title" value="{{ old('title') }}" class="admin-input" placeholder="e.g. Trending Now, New Arrivals, Best Sellers" required>
            </div>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
                <div class="admin-form-group">
                    ...
                    <select name="type" class="admin-input" id="sectionType" onchange="updateForm()" required>
                        <option value="trending" {{ old('type') === 'trending' ? 'selected' : '' }}>Trending (Featured Products)</option>
                        <option value="new_arrivals" {{ old('type') === 'new_arrivals' ? 'selected' : '' }}>New Arrivals</option>
                        <option value="best_selling" {{ old('type') === 'best_selling' ? 'selected' : '' }}>Best Sellers</option>
                        <option value="category" {{ old('type') === 'category' ? 'selected' : '' }}>By Category</option>
                        <option value="collection" {{ old('type') === 'collection' ? 'selected' : '' }}>By Collection</option>
                        <option value="manual" {{ old('type') === 'manual' ? 'selected' : '' }}>Manual Selection</option>
                ...
                <div class="admin-form-group">
                    ...
                    <select name="display_style" class="admin-input" required>
                        <option value="grid" {{ old('display_style') === 'grid' ? 'selected' : '' }}>Grid</option>
                        <option value="carousel" {{ old('display_style') === 'carousel' ? 'selected' : '' }}>Carousel</option>
                    </select>
                </div>
            </div>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px">
                <div class="admin-form-group">
                    ...
                    <select name="items_per_row" class="admin-input" required>
                        <option value="2" {{ old('items_per_row', 3) == 2 ? 'selected' : '' }}>2 columns</option>
                        <option value="3" {{ old('items_per_row', 3) == 3 ? 'selected' : '' }}>3 columns</option>
                        <option value="4" {{ old('items_per_row', 3) == 4 ? 'selected' : '' }}>4 columns</option>
                    </select>
                </div>
                ...
                <div class="admin-form-group">
                    ...
                    <label style="display:flex;align-items:center;gap:8px;cursor:pointer;margin-top:6px">
                        <input type="checkbox" name="is_enabled" value="1" {{ old('is_enabled', 1) ? 'checked' : '' }} style="opacity:0;width:0;height:0">
        ...
        <div class="admin-card__body" style="padding:0">
            <div id="productList" style="max-height:400px;overflow-y:auto">
                ...
                <label class="product-pick-item" data-category="{{ $product->category_id }}" data-collection="{{ $product->collections->pluck('id')->implode(',') }}"
                    ...
                    <input type="checkbox" name="product_ids[]" value="{{ $product->id }}" class="product-checkbox" style="width:16px;height:16px;flex-shrink:0">
    ...
    <div style="margin-top:20px;display:flex;gap:12px;max-width:800px">
        <button type="submit" class="btn-admin btn-admin--gold">Create Section</button>
...
function updateForm() {
    ...
    // Auto-show product picker for auto types
